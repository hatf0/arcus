...
void main(string[] args) {
	...
	{
		auto grabIP = executeShell("iptables -t nat -L PREROUTING | grep '" ~ args[1] ~ "' |  awk '{print $5}'");
		auto grabInternalIP = executeShell("iptables -t nat -L PREROUTING | grep '" ~ args[1] ~ "' |  awk '{print $9}'");
		...
		auto post = executeShell("iptables -t nat -L POSTROUTING --line-number | grep '" ~ args[1] ~ "' | awk '{print $1}'");
		auto pre = executeShell("iptables -t nat -L PREROUTING --line-number | grep '" ~ args[1] ~ "' | awk '{print $1}'");
