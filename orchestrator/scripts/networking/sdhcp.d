...
private:
	...
	alias DHCPInfo = Tuple!(bool, "status", dsin_addr, "ip", dsin_addr, "gateway", dsin_addr, "mask");
	bootp* bp;
...
public:
		...
		receive(
			(Variant v) {
				...
				if(type == DHCPack) {
					...
					optget(p.optdata, cast(ubyte*)&renewalTime, ODrenewaltime, renewalTime.sizeof); 
					optget(p.optdata, cast(ubyte*)&rebindingTime, ODrebindingtime, rebindingTime.sizeof);
	...
	this(string _bindiface = "wlan0", string _iface = "veth0", string hostname = "") {
		...
		{ 
			...
			if(setsockopt(sock.handle, SocketOptionLevel.SOCKET, 25, &_ifreq2, _ifreq2.sizeof) == -1){
