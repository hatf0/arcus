...
void main(string[] args) {
	...
	exec("/usr/bin/ip", "netns", "exec", args[1], "ifconfig", veth_pair[0],
			ipArrayToString(ip) ~ "/30");
	exec("/usr/bin/ip", "netns", "exec", args[1], "ip", "route", "add",
			"default", "via", ipArrayToString(ip_gateway));
	/* they're just executed inside of the network namespace for clarity sake */

	exec("/usr/bin/ip", "addr", "add", ipArrayToString(ip_gateway) ~ "/30",
			"brd", "+", "dev", "vm_bridge");
	...
	exec("/usr/bin/iptables", "-t", "nat", "-A", "POSTROUTING", "-s", ipArrayToString(ip),
			"-j", "SNAT", "--to-source", ipArrayToString(public_ip), "-m",
			"comment", "--comment", "\"" ~ args[1] ~ "\"");

	// forward all inbound requests for the public ip to the private ip
	exec("/usr/bin/iptables", "-t", "nat", "-A", "PREROUTING", "-j", "DNAT", "-d",
			ipArrayToString(public_ip), "--to-destination", ipArrayToString(ip),
			"-m", "comment", "--comment", "\"" ~ args[1] ~ "\"");
