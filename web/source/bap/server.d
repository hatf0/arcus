...
class BAPServer : Server
{
    private
    {
        ...
        */
        ...
        void pageRenderer(HTTPServerRequest req, HTTPServerResponse res,
                string templateName = "dashboard.dt")
        {
            User user;
            string lastError;
            string authKey;
            ...
            if (req.session)
            {
                ...
                if (templateName[0 .. 3] == "vps")
                {
                    ...
                    if (user.servers.canFind(req.params["vm_id"]))
                    {
                        ...
                        if (node.isNull)
                        {
                            req.session.set("last_error",
                                    "Your node appears to be down. Contact your administrator.");
                            res.redirect("/");
                            return;
                        }
                        import std.format;
                        ...
                        try
                        {
                            ...
                            authKey = cl.postVPS(node.communicationKey,
                                    req.params["vm_id"], "auth_key");
                            if (authKey == "NO")
                            {
                                logError(
                                        "got non-OK message for node '"
                                        ~ vps.node ~ "', consider investigating");
                                req.session.set("last_error", "The node '" ~ vps.node
                                        ~ "' failed to respond to our authentication request. Contact your administrator.");
                                res.redirect("/");
                            }
                        }
                        catch (Exception e)
                        {
                            ...
                            req.session.set("last_error",
                                    "The node '" ~ vps.node
                                    ~ "' appears to be down. Contact your administrator.");
                            res.redirect("/");
                            return;
                        }
            ...
            if (user.admin)
            {
                ...
                {
                    sidebarContents = generateSidebars([
            ...
            {
            case "dashboard.dt":
                res.render!("dashboard.dt", user, lastError, sidebarContents,
                        serverInterface, templateName);
                break;
            case "vps/dashboard.dt":
                res.render!("vps/dashboard.dt", user, lastError,
                        sidebarContents, serverInterface, templateName, vps, authKey);
                break;
            case "vps/disks.dt":
                res.render!("vps/disks.dt", user, lastError, sidebarContents,
                        serverInterface, templateName, vps, authKey);
                break;
            case "vps/advanced.dt":
                res.render!("vps/advanced.dt", user, lastError,
                        sidebarContents, serverInterface, templateName, vps, authKey);
                break;
            case "vps/network.dt":
                res.render!("vps/network.dt", user, lastError, sidebarContents,
                        serverInterface, templateName, vps, authKey);
                break;
            case "vps/redeploy.dt":
                res.render!("vps/redeploy.dt", user, lastError,
                        sidebarContents, serverInterface, templateName, vps, authKey);
                break;
            case "vps/statistics.dt":
                res.render!("vps/statistics.dt", user, lastError,
                        sidebarContents, serverInterface, templateName, vps, authKey);
                break;
            case "admin/dashboard.dt":
                res.render!("admin/dashboard.dt", user, lastError,
                        sidebarContents, serverInterface, templateName);
                break;
            case "admin/nodes.dt":
                res.render!("admin/nodes.dt", user, lastError,
                        sidebarContents, serverInterface, templateName);
                break;
            case "admin/users.dt":
                res.render!("admin/users.dt", user, lastError,
                        sidebarContents, serverInterface, templateName);
                break;
            case "admin/vms.dt":
                res.render!("admin/vms.dt", user, lastError, sidebarContents,
                        serverInterface, templateName);
                break;
            default:
                break;
        ...
        void admin_user_target(HTTPServerRequest req, HTTPServerResponse res)
        {
            if (req.params["action"] == "create")
            {
                ...
                enforceHTTP("username" in req.form && "email" in req.form && "fullname" in req.form
                        && "picture" in req.form, HTTPStatus.badRequest, "Missing form..");
        ...
        void admin_vm_target(HTTPServerRequest req, HTTPServerResponse res)
        {
            ...
            if (req.params["action"] == "provision")
            {
                enforceHTTP("node" in req.form && "hostname" in req.form && "vcpu_count" in req.form
                        && "ram_size" in req.form && "disk_size" in req.form
                        && "os_template" in req.form && "user" in req.form, HTTPStatus.badRequest,
                        "One or more fields were not submitted in the form.");
        ...
        {
            enforceHTTP("password-1" in req.form && "password-2" in req.form,
                    HTTPStatus.badRequest, "No passwords submitted..");
        ...
        void registerInternalPages()
        {
            router.get("/static/*", serveStaticFiles("public/", fileSettings))
                .get("/", &index).post("/login", &login).any("*", &verifyLoggedIn)
                .get("/logout", &logout).post("/resetpw", &reset_password).get("/general/:action", &dashboard)
                .get("/:vm_id/:action", &vm_dashboard).any("*", &verifyAdmin)
                .get("/admin/:action", &admin_panel) //                  .post("/admin/:action", &admin_do)
                .post("/admin/user/:action",
                        &admin_user_target).post("/admin/user/:target/:action",
                        &admin_user_target_do).post("/admin/node/:action",
                        &admin_node_target)
                .post("/admin/node/:target/:action", &admin_node_target_do)
                .post("/admin/vps/:action", &admin_vm_target).post("/admin/vps/:target/:action",
                        &admin_vm_target_do);
    ...
    public
    {
        ...
        void registerSidebar(string category, string displayName,
                string displayIcon, Sidebar[] entries)
        {
