...
struct file_entry {
	...
	bool writable; //assume it's only writable if NOT locked
	string name;
	ubyte[] buf;
...
class ResourceStorage : Operations {
	...
	@property shared Variant opIndex(string path)
	in {
	...
	@property shared Variant opIndexAssign(Variant dat, string path)
	in {
		assert(path in files, "Path did not exist in the array");
	...
	do {
		...
		assert(!locked, "Write attempted when resource was locked");
		try {
			if (dat.type == typeid(ubyte[])) {
	...
	override int write(const(char)[] _path, const(ubyte[]) buf, ulong offset) {
		...
		if (files.keys.canFind(path)) {
			...
			debug writefln("size of original array: %d, size of file array: %d",
					buf.length, file.buf.length);
		}
